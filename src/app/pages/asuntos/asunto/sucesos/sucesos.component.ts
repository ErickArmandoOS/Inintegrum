...
export class SucesosComponent implements OnInit {
  ...
  onSubmit(form: NgForm) {
    ...
    if (form.value.id == null)
      this.firestore.collection('sucesos').add(data);
    else
      this.firestore.doc('sucesos/' + form.value.id).update(data);
    this.resetForm(form);
    this.toastr.success('Submitted successfully', 'Suceso creado');
  }
